          ...
          {locations.map((loc, i) => (
            ...
            >
              ...
              <div className="flex items-start gap-2 text-sm text-slate-gray">
                ...
                <span className={loc.address === "Location TBA" ? "italic" : ""}>
                  {loc.address}
                </span>
              </div>
              ...
              {loc.stats ? (
                <div className="grid grid-cols-3 gap-3">
                  {loc.stats.map((s) => (
                    <div key={s.label} className="bg-frost-blue rounded-2xl p-3 text-center">
                      <p className="text-base font-black text-dark-navy">{s.value}</p>
                ...
                <p className="text-sm text-slate-gray italic">Details coming soon</p>
              )}

              {/* Status */}
              {loc.status ? (
                <div className="flex items-center gap-2 mt-auto">
                  <span className={`w-2 h-2 rounded-full ${loc.status.dot}`} />
